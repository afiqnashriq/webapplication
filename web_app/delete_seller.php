...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	if ($_POST['sure'] == 'Yes') {
		//Delete the record
		//make the query
		$q = "DELETE FROM seller WHERE seller_id = $id LIMIT 1";
		...
		if (mysqli_affected_rows($connect) == 1) {
			...
			//display message
			echo '<h3>The seller has been deleted.</h3>';
		} else {
			//display message
			echo '<p class="error">The parcel could not be deleted.<br>Probably because it does not exist or due to system error.</p>';

			echo '<p>' . mysqli_error($connect) . '<br>Query:' . $q . '</p>';
			//debugging message
		}
	} else {
		echo '<h3>The seller has NOT been deleted.</h3>';
	}
} else {
	//display the form
	//Retrieve the member's data
	$q = "SELECT name FROM seller WHERE seller_id = $id";
	$result = @mysqli_query($connect, $q);
